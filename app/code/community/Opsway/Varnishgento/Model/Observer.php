...
class Opsway_Varnishgento_Model_Observer
{
    ...
    public function processOutOfStockProducts($observer)
    {
        ...
        if ($this->_isStockProcessed === false) {
            ...
            $orders = array();
            $o = $observer->getEvent()->getOrder();
            if (isset($o)) { // onepage checkout
                $orders[] = $o;
            }
            else { // multi checkout Mage::dispatchEvent('checkout_submit_all_after', array('orders' => $orders, 'quote' => $this->getQuote()));
