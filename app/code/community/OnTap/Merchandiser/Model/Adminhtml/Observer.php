...
class OnTap_Merchandiser_Model_Adminhtml_Observer
{
    ...
    public function reindexCron()
    {
        ...
        // If Rebuild on Category Save / Product Save are selected, only rebuild if there are entries in the VM tables
        if (Mage::helper('merchandiser')->rebuildOnCategorySave() || Mage::helper('merchandiser')->rebuildOnProductSave())
        {
            if ( sizeof($vmBuildAttributeCodes) > 0 )
            {
            	$vmBuildAttributeCodes = array_map(array($this,"mapVmBiudRows"), $vmBuildAttributeCodes);
	...
	*/
    ...
    public function categorySaveAfter($observer)
    {
        ...
        if (sizeof($insertData) > 0) {
            Mage::getModel('merchandiser/merchandiser')->manipulateCategoryProductsAfterCategorySave($insertData, $catId);
