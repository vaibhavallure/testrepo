...
class Belvg_Ddmenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    /**
    ...
    protected function _toHtml()
    {
        if (Mage::helper('ddmenu')->isEnabled()) {
            $this->setTemplate('ecp/ddmenu/navigation/top.phtml');
        }
        ...
        if ($this->maxRows) {
            ...
            if ($children->count()) {
                ...
                foreach ($children as $child) {
                    $this->categoryIds[] = $child->getId();
                    $subHtml             = $this->getSubCategoryHtml($child, FALSE);
                    $i += 1 + (int)$this->rows;
                    if ($i > $this->maxRows && $boo && $first=='') {
                        $i           = 0;
                        $html       .= '</ul></td><td><ul>';
                        if ($boo) {
                            $first   = ' first';
