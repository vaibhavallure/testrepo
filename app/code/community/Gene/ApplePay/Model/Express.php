...
/**
 * Class Gene_ApplePay_Model_Express
 *
 * @author Dave Macaulay <dave@gene.co.uk>
 */
class Gene_ApplePay_Model_Express extends Mage_Core_Model_Abstract
{
    /**
    ...
    public function getAdditionalResponse($response)
    {
        ...
        if (isset($totals['discount']) && $totals['discount']->getValue() < 0) {
            $label = Mage::helper('gene_applepay')->__('Discount%s', ($quote->getCouponCode() ? ' (' . $quote->getCouponCode() . ')' : ''));
