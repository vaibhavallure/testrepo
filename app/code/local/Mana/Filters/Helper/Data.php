...
class Mana_Filters_Helper_Data extends Mana_Core_Helper_Layer {
    ...
    public function getGridModeUrl($modUrl, $_code) {
    	$url = $_SERVER['REQUEST_URI'];
    	$string_url = '';
    
    	if(isset($_GET['q']) && $_GET['q'] != '') {
    		$p = (isset($_GET['p']) && $_GET['p'] > 0 ? '&p='.$_GET['p'] : '');
    		$string_url = $modUrl. $p;
    	}
    	else {
    		$url = explode('/', $url);
    		$z = 0;
    		$found = 0;
    		$config_where = Mage::getStoreConfig('mana_filters/seo/conditional_word');
    		...
    		foreach($url as $k => $u) {
    			if($u != '') {
    				...
    				if($found > 0 && $k == ($found+1) ) {
    					$ext = (strpos($u,'.html') !== false ? '.html' : '');
    					$u = strtolower($_code) . $ext;
    		...
    		if(!$replaced) {
    			if (strpos($string_url,$config_where) !== false) {
    				$string_url = str_replace($config_where, $config_where.'/mode/'.strtolower($_code), $string_url);
    			}
    			else {
    				if (strpos($string_url,'mode') !== false) {
    					$mode = $_GET['mode'];
    					$string_url = str_replace($mode, strtolower($_code), $string_url );
    				}
    				else {
    					$string_url = str_replace('.html', '/'.$config_where.'/mode/'.strtolower($_code).'.html', $string_url);
    				}
    			}
    		}
    		$string_url = Mage::getBaseUrl() . $string_url;
    	}
    	return $string_url;
    }
}