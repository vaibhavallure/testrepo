...
class Alluremultistore_Catalog_Adminhtml_Catalog_Product_Action_AttributeController extends Mage_Adminhtml_Catalog_Product_Action_AttributeController
{
    ...
    public function saveAction()
    {
        ...
        //retrive stock id by using website id
        $stockIdArr = array();
        if(count($websiteAddData)>0){
        	$websiteModel = Mage::getModel('core/website');
        	foreach ($websiteAddData as $websiteId){
        		$stockId = $websiteModel->load($websiteId)->getStockId();
        		$stockIdArr[$websiteId] = $stockId;
        	}
        }
        ...
        try {
        	...
        	//allure code start
        	$path = Mage::getBaseDir('media') . DS . 'catalog' . DS . 'product' . DS. 'thumbnail' . DS . 'video' . DS;
        	if(!empty($_FILES['video_thumb']['name']) || !empty($_FILES['second_video_thumb']['name'])){
        		$viedoThumbName = "";
        		$secondViedoThumbName = "";
        		if(!empty($_FILES['video_thumb']['name'])){
	        		$key = round(microtime(true)*1000);
	        		$ext = explode(".", $_FILES['video_thumb']['name']);
	        		$viedoThumbName = $key.".".$ext[1];
	        		move_uploaded_file($_FILES["video_thumb"]["tmp_name"],$path.$viedoThumbName);
        		}
        		if(!empty($_FILES['second_video_thumb']['name'])){
        			$key = round(microtime(true)*1000);
        			$ext = explode(".", $_FILES['second_video_thumb']['name']);
        			$secondViedoThumbName = $key.".".$ext[1];
        			move_uploaded_file($_FILES["second_video_thumb"]["tmp_name"],$path.$secondViedoThumbName);
        		}
        		
        		foreach ($this->_getHelper()->getProductIds() as $productId) {
	        		$product = Mage::getModel('catalog/product')->load($productId);
	        		if(!empty($viedoThumbName))
	        			$product->setVideoThumb($viedoThumbName);
	        		
	        		if(!empty($secondViedoThumbName))
	        			$product->setSecondVideoThumb($secondViedoThumbName);
            ...
            if ($inventoryData) {
                ...
                foreach ($this->_getHelper()->getProductIds() as $productId) {
                    ...
                    //add stock id by website id
                    if(count($stockIdArr)>0){
                    	foreach ($websiteAddData as $websiteId){
		                    $stockItem->loadByProductAndStock($productId,$stockIdArr[$websiteId])
