...
class Alluremultistore_Catalog_Helper_Adminhtml 
    extends Mage_Core_Helper_Abstract 
{
    ...
    {
        return Mage::helper('alluremultistore_catalog');
    }
    /**
     ...
     * @return self
     */
    protected function addColumnBatchPriceDataToCollection($collection, $column,$store)
    {
    	if($store!=0){
    		$websiteId = Mage::getModel("core/store")->load($store)->getWebsiteId();
    		$websites = array();
       ...
       // $priceHelper    = $helper->getProductPriceHelper();
        ...
        foreach ($collection as $product) {
            ...
            foreach ($stockIds as $stockId) {
            	$websiteId = Mage::getModel("core/website")->load($stockId,'stock_id')->getWebsiteId();
            	if(in_array($websiteId, $websites)){
	                 $_product = Mage::getModel('catalog/product')->setStoreId($storeIds[$stockId])
	                    ->load($product->getId());
    ...
    public function addQtyProductGridColumn($grid)
    {
        ...
        $grid->addColumn('qtys', array(
            ...
            'width'         => '140px', 
            'align'         => 'left', 
            'renderer'      => 'alluremultistore_catalog/adminhtml_catalog_product_grid_column_renderer_qtys', 
    ...
    public function addBatchPriceProductGridColumn($grid)
    {
        ...
        $grid->addColumn('batch_prices', array(
            ...
            'width'         => '140px', 
            'align'         => 'left', 
            'renderer'      => 'alluremultistore_catalog/adminhtml_catalog_product_grid_column_renderer_batchprices', 
            'filter'        => false, 
            'sortable'      => false, 
        ));
        return $this;
    ...
    public function prepareProductGrid($grid,$store)
    {
        ...
        if ($config->isCatalogBackendGridQtyVisible()) {
            $qtyColumnId = 'qtys';
            $this->addColumnQtyDataToCollection($grid->getCollection(), $grid->getColumn($qtyColumnId),$store);
            $grid->addColumnsOrder($qtyColumnId, 'price');
        }
        if ($config->isCatalogBackendGridBatchPricesVisible()) {
            $batchPricesColumnId = 'batch_prices';
            $this->addColumnBatchPriceDataToCollection($grid->getCollection(), $grid->getColumn($batchPricesColumnId),$store);
