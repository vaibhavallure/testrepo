...
{
    protected  $_pagesCollection;
    protected  $_pagesCount;
    ...
    {
        return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'search/category/jewelry';
    }
    ...
    public  function  getCategoryCollection(){
        $query_txt = $this->getQueryText();
        $elastic_helper = $this->getElasticHelper();

        if( ($elastic_helper != null) &&  (strlen(trim($query_txt)) > 0) ){
            $this->_categoriesCollection = $elastic_helper->getCategoryCollection($query_txt);
            $this->_categoriesCount = $this->_categoriesCollection->getSize();
    ...
    public function  getPageCollection(){
        ...
        if( ($elastic_helper != null) &&  (strlen(trim($query_txt)) > 0) ){
            $this->_pagesCollection = $elastic_helper->getPageCollection($query_txt);
            $this->_pagesCount = $this->_pagesCollection->getSize();
            return $this->_pagesCollection;
        }
        return null;
    }

    public  function getElasticHelper(){

        $isEnable = Mage::helper('core')->isModuleEnabled('Wyomind_Elasticsearch');
        if($isEnable)
        {
