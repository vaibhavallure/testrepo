...
class Mage_Customer_Model_Session extends Mage_Core_Model_Session_Abstract
{
    ...
    {
        return (bool)$this->getId() && (bool)$this->checkCustomerId($this->getId()) && Mage::getModel('core/cookie')->get('frontend_timeout');
    }
    ...
    public function login($username, $password)
    {
        ...
        if ($customer->authenticate($username, $password)) {
            ...
            //$this->renewSession();
			Mage::getModel('core/cookie')->set('frontend_timeout', 1, Mage::getStoreConfig('web/cookie/cookie_lifetime2'));
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->isLoggedIn() && Mage::app()->getRequest()->getActionName() == 'logout') {
            return true;
        }
		...
		if(Mage::getSingleton('customer/session')->isLoggedIn()) {
			if(!Mage::getModel('core/cookie')->get('frontend_timeout') && Mage::app()->getRequest()->getControllerName() == 'multishipping')
			Mage::getModel('core/cookie')->set('from_multi_checkout_page', 1);
			$action->getResponse()->setRedirect(Mage::getUrl('customer/account/login', array('_current' => true)));
            return false;
		}
