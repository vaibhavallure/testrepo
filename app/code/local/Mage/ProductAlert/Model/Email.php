...
class Mage_ProductAlert_Model_Email extends Mage_Core_Model_Abstract
{
    ...
    protected $_stockProducts2 = array();
    ...
    public function addStockProduct2(Mage_Catalog_Model_Product $product)
    {
        $this->_stockProducts2[$product->getId()] = $product;
        return $this;
    }
    ...
    protected function _getStockBlock2()
    {
        if (is_null($this->_stockBlock2)) {
            $this->_stockBlock2 = Mage::helper('productalert')
                ->createBlock('productalert/email_stock');
        }
        return $this->_stockBlock2;
    }
    ...
    public function send2() {
        ...
        $this->_getStockBlock()
                ->setStore($store)
                ->reset();
        foreach ($this->_stockProducts2 as $product) {
            //$product->setCustomerGroupId($this->_customer->getGroupId());
            $this->_getStockBlock2()->addProduct($product);
        }
        $block = $this->_getStockBlock2()->toHtml();
        $store = $this->_website->getDefaultStore();
        $storeId = $store->getId();
        $templateId = Mage::getStoreConfig(self::XML_PATH_EMAIL_STOCK_TEMPLATE, $storeId);
        $customer = Mage::getModel("customer/customer");
        $customer->setWebsiteId(Mage::app()->getWebsite()->getId());
        $customer->loadByEmail($this->_email); //load customer by email id
        $customerName = ($customer->getName() != ' ') ? $customer->getName() : 'Guest';
        Mage::getModel('core/email_template')
                ->setDesignConfig(array(
                    'area' => 'frontend',
                    'store' => $storeId
                ))->sendTransactional(
                $templateId, Mage::getStoreConfig(self::XML_PATH_EMAIL_IDENTITY, $storeId), $this->_email, $customerName, array(
