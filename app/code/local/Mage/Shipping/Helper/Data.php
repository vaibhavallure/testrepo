...
class Mage_Shipping_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getHashByOrderId($order = ''){
        if ($order && !is_object($order)) {
            $order = Mage::getModel('sales/order')->load($order);
        }

        return $this->_getHash('order_id', $order);
    }

    public function getHashByShipmentId($ship = ''){
        if ($ship && !is_object($ship)) {
            $ship = Mage::getModel('sales/order_shipment')->load($ship);
        }

        return $this->_getHash('ship_id', $ship);
    }

    public function getHashByTrackId($track = '')
    {
        if ($track && !is_object($track)) {
            $track = Mage::getModel('sales/order_shipment_track')->load($track);
        }

        return $this->_getHash('track_id', $track, 'getEntityId');
    }

    protected function _getHash($key, $model, $method = 'getId'){
        return Mage::helper('core')->urlEncode("{$key}:{$model->$method()}:{$model->getProtectCode()}");
    }
