...
class Mage_Core_Model_Store extends Mage_Core_Model_Abstract
{
     ...
     *
     * this function customized to get appropriate base currency using geo location
     *
     */
    public function getBaseCurrency($customAttr=false)
    {


        if(Mage::helper('core')->isModuleEnabled('Allure_MultiCurrency') && $customAttr) {
            $currency = Mage::getModel('directory/currency')->load(Mage::Helper('multicurrency')->getCurrentCountryCurrencyCode());
     ...
     * @return  double
     */
    public function convertPrice($price, $format = false, $includeContainer = true, $customAttr = false)
    {


        if ($this->getCurrentCurrency() && $this->getBaseCurrency()) {
               $value = $this->getBaseCurrency($customAttr)->convert($price, $this->getCurrentCurrency());
           } else {
               $value = $price;
           }
