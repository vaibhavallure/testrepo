...
class Mage_Catalog_Helper_Product_Type_Composite extends Mage_Core_Helper_Abstract
{
     ...
     * @return float
     */
    public function convertPrice($price, $round = false, $storeId = null,$customAttr=false)
    {
        if (empty($price)) {
            return 0;
        }

        $price = $this->getCurrentStore($storeId)->convertPrice($price,false,true,$customAttr);
        if ($round) {
