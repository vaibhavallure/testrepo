...
{
    public function getChildPrice($oParent, $oProduct, $qty){
        $price = $this->getBasePrice($oParent, 1);

        // Get configurable product attributes
        $configurableAttributes = $oParent->getTypeInstance(true)->getConfigurableAttributesAsArray($oParent);

        // Get configurable products combination
        foreach($configurableAttributes as $configAttribute){
            $sAttributeCode = $configAttribute['attribute_code'];
            $sAttributeValue = $oProduct->getResource()->getAttribute($sAttributeCode)->getFrontend()->getValue($oProduct);

            foreach($configAttribute['values'] as $configAttributeDetails){
                if($sAttributeValue == $configAttributeDetails['label']){
                    $price += (float)$configAttributeDetails['pricing_value'];
                }
            }
        }
