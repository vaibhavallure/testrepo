   ...
   /**
    ...
    protected function _exportLineItems(array &$request, $i = 0)
    {
    	...
    	if($_checkoutstepHelper->isTwoShipment()){
            ...
            Mage::log($totals,Zend_log::DEBUG,'abc.log',true);
    	...
    	Mage::log("before request - ",Zend_log::DEBUG,'abc.log',true);
    	...
    	foreach ($items as $item) {
    	    Mage::log("item in - ".$item->getId(),Zend_log::DEBUG,'abc.log',true);
    		foreach ($this->_lineItemExportItemsFormat as $publicKey => $privateFormat) {
    			...
    			Mage::log("private format - ".sprintf($privateFormat, $i),Zend_log::DEBUG,'abc.log',true);
    		}
    		$i++;
    	}
    	
    	Mage::log($items,Zend_log::DEBUG,'abc.log',true);
    	Mage::log("2 ship - ",Zend_log::DEBUG,'abc.log',true);
    	Mage::log($request,Zend_log::DEBUG,'abc.log',true);
    	
    	if($isTwoShip){
    	    if(!empty($totals['discount'])){
    	        $paypalCart->isDiscountAsItem(true);
    	    }
    	    
    	    $items1 = $paypalCart->getItems();
    	    Mage::log($items1,Zend_log::DEBUG,'abc.log',true);
    	    if (empty($items1) || !$this->getIsLineItemsEnabled()) {
    	        return;
    	    }
    	    $result = null;
    	    foreach ($items1 as $item) {
    	        Mage::log("item out - ".$item->getId(),Zend_log::DEBUG,'abc.log',true);
    	        foreach ($this->_lineItemExportItemsFormat as $publicKey => $privateFormat) {
    	            $result = true;
    	            $value = $item->getDataUsingMethod($publicKey);
    	            if (isset($this->_lineItemExportItemsFilters[$publicKey])) {
    	            ...
    	            $request[sprintf($privateFormat, $i)] = $value;
    	            Mage::log("private format new - ".sprintf($privateFormat, $i),Zend_log::DEBUG,'abc.log',true);
    	        }
    	        $i++;
    	    }
    	}
