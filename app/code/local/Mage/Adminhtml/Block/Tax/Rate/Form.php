...
class Mage_Adminhtml_Block_Tax_Rate_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset->addField('is_min_tax_amount', 'select', array(
            'name'    => 'is_min_tax_amount',
            'label'   => Mage::helper('tax')->__('Is Min Tax Amount Limit'),
            'options' => array(
                '0' => Mage::helper('tax')->__('No'),
                '1' => Mage::helper('tax')->__('Yes'),
            )
        ));
        $fieldset->addField('min_tax_amount', 'text', array(
            'name'     => 'min_tax_amount',
            'label'    => Mage::helper('tax')->__('Min Tax Amount'),
            'title'    => Mage::helper('tax')->__('Min Tax Amount'),
            'class'    => 'validate-not-negative-number'
        ));
