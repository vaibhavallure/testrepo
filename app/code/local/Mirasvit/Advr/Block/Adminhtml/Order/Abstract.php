...
{
    protected $invoiceIncompatibleColumns = array(
        'sum_total_invoiced',
        'avg_total_invoiced',
    );
     ...
     * @return array
     */
    protected function convertToInvoiceColumns(array $columns)
    {
        // unset columns incompatible with invoice table
        foreach ($this->invoiceIncompatibleColumns as $column) {
            unset($columns[$column]);
        }

        // convert order columns to invoice
        foreach ($columns as $key => $column) {
            $invoiceKey = 'invoice_' . $key;

            if (isset($column['index'])) {
                $column['index'] = 'invoice_' . $column['index'];
            }

            $column['header'] = str_replace('Order', 'Invoice', $column['header']);
            $columns[$invoiceKey] = $column;
