...
class Magestore_Webpos_Block_Adminhtml_Zreport_Grid extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _prepareCollection() {
        $collection = Mage::getModel('webpos/zreport')->getCollection();
    ...
    protected function _prepareColumns() {
        ...
        $this->addColumn('till_id', array(
            'header' => Mage::helper('webpos')->__('Cash Drawer'),
            'align' => 'left',
            'index' => 'till_id',
            'renderer'  => 'Magestore_Webpos_Block_Adminhtml_Zreport_Renderer_Cashdrawer',
            'type' => 'options',
            'options' => $this->getCashDrawer()
        ));
        ...
        $this->addColumn('opening_amount', array(
            'header' => Mage::helper('webpos')->__('Opening amount'),
            'align' => 'left',
            'index' => 'opening_amount',
            'type' => 'currency',
    ...
    public function getCashDrawer(){
        ...
        if($collection->getSize() > 0){
            foreach ($collection as $till){
                $locations[$till->getId()] = $till->getTillName();
            }
        }
        return $locations;
    }
