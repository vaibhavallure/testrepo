...
class Varien_Filter_Template implements Zend_Filter_Interface
{
    ...
    public function tattooDirective($construction) {
        if(!$tattooer = Mage::registry('tattooer')){
            $tattooer = Mage::helper('ecp_tattoo')->setTattooer();
        }
        return $tattooer->getData(trim($construction[2]));
    }
