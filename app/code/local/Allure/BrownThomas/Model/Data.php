...
class Allure_BrownThomas_Model_Data
{
    ...
    public function getFITEM()
    {
        ...
        foreach ($products as $product_id) {
            ...
            $data[$product_id]['unit_of_measure'] = $this->formatString("EA", 6);
    ...
    public function getFUDAS()
    {
        ...
        foreach ($products as $product_id) {
            $_product = Mage::getSingleton("catalog/product")->load($product_id);
            $data[$product_id]['record_type'] = 'FUDAS';
            $data[$product_id]['action_type'] = 'N';
            $data[$product_id]['UPC'] = $this->formatString($_product->getBarcode(), 13);
            $data[$product_id]['UDA_name'] = $this->formatString('web_enabled_uda', 16);
            $data[$product_id]['UDA_value'] = $this->formatString(1, 250);
