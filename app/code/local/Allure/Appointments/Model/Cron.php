...
class Allure_Appointments_Model_Cron extends Mage_Core_Model_Abstract
{
    ...
    public function processCollection($storeDate, $storeId){
        ...
        Mage::log(" processCollection  storeDate=>".$storeDate." storeId=>".$storeId,Zend_Log::DEBUG,'notify_cron.log',true);
        ...
        Mage::log(" Send notification before one day",Zend_Log::DEBUG,'notify_cron.log',true);
        Mage::log("from=>".$nextTime." to=>".$next2Time,Zend_Log::DEBUG,'notify_cron.log',true);
        ...
        {
            Mage::log("appointments not found",Zend_Log::DEBUG,'notify_cron.log',true);
        }
        Mage::log("Check more than 7 day remaining appointments",Zend_Log::DEBUG,'notify_cron.log',true);
        $nextTime = date("Y-m-d H:i:00",strtotime("7 day",strtotime($storeDate)));
        Mage::log("nextTime After 7 day:".$nextTime,Zend_Log::DEBUG,'notify_cron.log',true);
        $next2Time= date("Y-m-d H:i:59",strtotime("7 day 15 minutes",strtotime($storeDate)));
        Mage::log("$next2Time After 7 day 15 minutes:".$nextTime,Zend_Log::DEBUG,'notify_cron.log',true);
        $allAppointments = Mage::getModel('appointments/appointments')->getCollection();
        $allAppointments->addFieldToFilter('app_status',Allure_Appointments_Model_Appointments::STATUS_ASSIGNED);
        $allAppointments->addFieldToFilter('appointment_start', array('gteq' => $nextTime));
        $allAppointments->addFieldToFilter('appointment_start', array('lteq' => $next2Time));
            ...
            if($sendEmail){
                ...
                if($toSend){
                    ...
                    $vars = array(
                        ...
                        'store_address'	=>$storeaddress,//Mage::getStoreConfig("appointments/genral_email/store_address",$storeId),
