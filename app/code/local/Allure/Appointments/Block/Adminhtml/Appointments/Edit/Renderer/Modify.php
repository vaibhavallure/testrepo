	...
	public function render (Varien_Object $row)
	{
	    ...
	    if(!empty($row->getId()) && !empty($row->getEmail()))
	    {
            if($row->getSpecialStore()) {
                $getCustomersInfo = Mage::helper('appointments/data')->getCustomersInfoSetting($row->getStoreId());
                if($getCustomersInfo)
                {
                    echo '<a target="_blank"  href=' . $this->getUrl('*',array('id'=>$row->getId(),'email'=>$row->getEmail(),'store'=>$row->getStoreId())). '>Modify</a>';
