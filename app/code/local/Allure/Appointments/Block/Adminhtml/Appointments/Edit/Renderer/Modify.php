    ...
    public function render(Varien_Object $row)
    {
        ...
        if (!empty($row->getId()) && !empty($row->getEmail())) {
            if ($row->getSpecialStore()) {
                ...
                if ($row->getSpecialStore() == 2) {
                    echo '<a target="_blank"  href=' . Mage::helper('adminhtml')->getUrl('admin_appointments/adminhtml_appointments/newsystem/user/admin/id/' . $row->getId()), array('_secure' => true, 'key' => $key) . '>Modify</a>';
                } else {
