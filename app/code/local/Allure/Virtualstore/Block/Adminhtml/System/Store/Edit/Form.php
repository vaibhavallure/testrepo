...
class Allure_Virtualstore_Block_Adminhtml_System_Store_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($showWebsiteFieldset) {
            ...
            if (Mage::registry('store_action') == 'edit') {
                $groups = Mage::getModel('allure_virtualstore/group')->getCollection()
                    ->addFieldToFilter("website_id",$websiteModel->getId())
                    ->addFieldToFilter('group_id', array('gt' => 0))//->setWithoutStoreViewFilter()
                    ->toOptionArray();

                $fieldset->addField('website_default_group_id', 'select', array(
                    'name'      => 'website[default_group_id]',
                    'label'     => Mage::helper('allure_virtualstore')->__('Default Store'),
            ...
            if (!$websiteModel->getIsDefault() && $websiteModel->getStoresCount()) {
                $fieldset->addField('is_default', 'checkbox', array(
                    ...
                    'label'     => Mage::helper('allure_virtualstore')->__('Set as Default'),
                    'value'     => 1,
                    'disabled'  => $websiteModel->isReadOnly(),
                ));
            }
        ...
        if ($showGroupFieldset) {
            ...
            $fieldset = $form->addFieldset('group_fieldset', array(
                'legend' => Mage::helper('allure_virtualstore')->__('Store Information')
            ));

            if (Mage::registry('store_action') == 'edit'
                || (Mage::registry('store_action') == 'add' && Mage::registry('store_type') == 'group')) {
                $websites = Mage::getModel('allure_virtualstore/website')
                ->getCollection()->toOptionArray();
                $fieldset->addField('group_website_id', 'select', array(
                    'name'      => 'group[website_id]',
                    'label'     => Mage::helper('allure_virtualstore')->__('Website'),
                    'value'     => $groupModel->getWebsiteId(),
                    'values'    => $websites,
                    'required'  => true,
                    'disabled'  => ($groupModel->getWebsiteId())? true : false,//$groupModel->isReadOnly(),
                ));
            ...
            $fieldset->addField('group_root_category_id', 'select', array(
                ...
                'label'     => Mage::helper('allure_virtualstore')->__('Root Category'),
        ...
        if ($showStoreFieldset) {
            ...
            $fieldset = $form->addFieldset('store_fieldset', array(
                'legend' => Mage::helper('allure_virtualstore')->__('Store View Information')
            ));

            if (Mage::registry('store_action') == 'edit'
                || Mage::registry('store_action') == 'add' && Mage::registry('store_type') == 'store') {
                $websites = Mage::getModel('allure_virtualstore/website')->getCollection();
                $allgroups = Mage::getModel('allure_virtualstore/group')->getCollection();
                $groups = array();
                ...
                $fieldset->addField('store_group_id', 'select', array(
                    'name'      => 'store[group_id]',
                    'label'     => Mage::helper('allure_virtualstore')->__('Store'),
            ...
            $fieldset->addField('store_is_active', 'select', array(
                ...
                'label'     => Mage::helper('allure_virtualstore')->__('Status'),
