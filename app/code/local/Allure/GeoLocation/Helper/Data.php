<?php
class Allure_GeoLocation_Helper_Data extends Mage_Core_Helper_Abstract
{
    const XML_PATH_CURL_TIMEOUT = 'allure_geolocation/general/curl_timeout';
    
    public function getConfig($key)
    {        
        return Mage::getStoreConfig('allure_geolocation/general/'.$key);
    }
    ...
    {
        return $this->getConfig('country_override');
    }
    
    public function getIpOverride()
    {
        return $this->getConfig('ip_override');
    }
    ...
    {
        if ($this->getConfig('private_mode')){
            return false;
        }
    ...
    {        
        $ipList = str_replace(" ", "", $this->getConfig('ip_whitelist'));
    ...
    {             
        if ($this->getConfig('proxy_enabled')){
            $headers = str_replace(" ", "", $this->getConfig('proxy_headers'));
            $headerArr = explode(",", $headers);
            for($i=0; $i<count($headerArr);$i++){
                Mage::getConfig()->setNode(Mage_Core_Helper_Http::XML_NODE_REMOTE_ADDR_HEADERS.'/header'.$i+1, $headerArr[$i]);
            }                  
        }
    }
