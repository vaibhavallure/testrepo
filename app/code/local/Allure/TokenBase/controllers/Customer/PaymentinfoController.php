		...
		if( $this->_formKeyIsValid() === true && $this->_methodIsValid() === true ) {
			...
			try {
				...
				if( $card && ( $id == 0 || ( $card->getId() == $id && $card->hasOwner( $customer->getId() ) ) ) ) {
					...
					$this->loadLayout('myaccount_pay_card_data');
					$html = $this->getLayout()->getBlock('tokenbase_customer_wrapper')->toHtml();
					
					$result['html'] = $html;
					$result['success'] = 1;
					$result['message'] = $this->__('Payment data saved successfully.');
				}
				else {
					$result['success'] = 0;
		...
		if( $this->_formKeyIsValid() === true && $this->_methodIsValid() === true && $id > 0 ) {
			try {
				...
				if( $card && $card->getId() == $id && $card->hasOwner( Mage::helper('tokenbase')->getCurrentCustomer()->getId() ) ) {
					...
					$result['html'] = $html;
					$result['success'] = 1;
					$result['message'] = $this->__('Payment record deleted.');
				}
				else {
					$result['success'] = 0;
					$result['message'] = $this->__('Invalid Request.');
	...
	public function editAjaxAction(){
		...
		if( $this->_formKeyIsValid() === true && $this->_methodIsValid() === true && $id > 0 ) {
			if( $id > 0 ) {
				...
				$this->loadLayout('myaccount_pay_card_edit');
				$html = $this->getLayout()->getBlock('tokenbase_manage_authnetcim_form')->toHtml();
				...
				$result['success'] = 0;
				$result['message'] = $this->__('1 No payment methods are currently available.');
			}
		}else {
			$result['success'] = 0;
			$result['message'] = $this->__('3 No payment methods are currently available.');
		}
