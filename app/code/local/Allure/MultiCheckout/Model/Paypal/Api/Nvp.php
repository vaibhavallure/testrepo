<?php
class Allure_MultiCheckout_Model_Paypal_Api_Nvp extends Mage_Paypal_Model_Api_Nvp
{
	/**
	...
	protected function _exportLineItems(array &$request, $i = 0)
	{
        ...
        Mage::log("nvp class",Zend_log::DEBUG,'ajay.log',true);
		...
		if ($this->_lineItemTotalExportMap) {
			foreach ($this->_cart->getTotals() as $key => $total) {
				if (isset($this->_lineItemTotalExportMap[$key])) { // !empty($total)
					...
					Mage::log("cart 2 - ".$privateKey."=".$request[$privateKey],Zend_log::DEBUG,'abc.log',true);
				}
			}
		}
