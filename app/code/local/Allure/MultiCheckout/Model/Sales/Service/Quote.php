...
class Allure_MultiCheckout_Model_Sales_Service_Quote extends Mage_Sales_Model_Service_Quote
{
     ...
     */
    public function submitOrderForSingleAddress()
    {
        // don't allow submitNominalItems() to inactivate quote
        $shouldInactivateQuoteOld = $this->_shouldInactivateQuote;
        $this->_shouldInactivateQuote = false;
        ...
        //get gift message id for shipping address
        $addressGiftItem = array();
        $shippingAddress = $quote->getShippingAddress();
        foreach($shippingAddress->getAllItems() as $item){
            if(!$item->getGiftMessageId()) continue;
            $addressGiftItem[$item->getQuoteItemId()] = $item->getGiftMessageId();
        }
        
        foreach ($quote->getAllItems() as $item) {
            //set gift message for single address
            if(isset($addressGiftItem[$item->getId()])){
                $item->setGiftMessageId($addressGiftItem[$item->getId()]);
            }
