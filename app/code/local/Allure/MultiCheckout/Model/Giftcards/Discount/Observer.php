 ...
 */
class Allure_MultiCheckout_Model_Giftcards_Discount_Observer extends Webtex_Giftcards_Model_Discount_Observer
{
            ...
            if ($this->_oQuote->getOrderType() != $_checkoutstepHelper::MULTI_BACK_ORDER) {
                ...
                foreach ($aAddresses as $oAddress) {
                    if ($oAddress->getGrandTotal() > 0) {
                        ...
                        // if after shipping discount calculation gift card
                        // balance become 0,
                        ...
                        // if after sets address total gift card balance become
                        // 0,
                        // there is no need to continue calculate discount for
                        // other addresses
    ...
    private function _setShippingDiscount ()
    {
        ...
        if ($shippingAmount > 0) {
            ...
            if (($shippingAmount - $shippingDiscount - $this->_giftCardBalance) >= 0) {
                ...
                $continueDiscountCalculation = false; // giftcardbalance = 0, so
    ...
    private function _setDiscountDescription ()
    {
        ...
        if ($this->_oAddress->getDiscountDescription()) {
            $discDescr = $this->_oAddress->getDiscountDescription() . ', Gift Card ' . $description; // -
                                                                                                   // not
                                                                                                   // show
                                                                                                   // id
                                                                                                   // gift
    ...
    private function _setItemsDiscount ()
    {
        ...
        $availableDiscount = $this->_giftCardBalance - $this->_shippingDiscount; // +
                                                                                 // $this->_shippingDiscountAdditional;
        $baseAvailableDiscount = $this->_baseGiftCardBalance - $this->_baseShippingDiscount; // +
                                                                                             // $this->_baseShippingDiscountAdditional;
        ...
        $this->_oAddress->setBaseDiscountAmount(
                - ($baseDiscount + $this->_baseShippingDiscount + $baseItemDiscountAmount));
    }
    ...
    private function _setBalanceByCurrencies ($giftCardsIds)
    {
        ...
        $baseBalance = 0;
        $balanceForFront = 0;
        $cards = Mage::getModel('giftcards/giftcards')->getCollection()->addFieldToFilter('card_id',
                array(
                        'in' => $giftCardsIds
                ));
        ...
        foreach ($cards as $card) {
            ...
            if ($baseCurrency == $currentCurrency) {
                ...
                if ($baseCurrency == $cardCurrency) {
                    ...
                    $baseBalance += Mage::helper('giftcards')->currencyConvert($card->getCardBalance(), $currentCurrency,
                            $baseCurrency);
                    $balance += $card->getCardBalance();
                } else {
                    $baseBalance += Mage::helper('giftcards')->currencyConvert($card->getCardBalance(), /*from*/ $cardCurrency, /*to*/$baseCurrency);
                    $balance = Mage::helper('giftcards')->currencyConvert($baseBalance, /*from*/ $baseCurrency, /*to*/$currentCurrency); // from
                                                                                                                                         // base
                                                                                                                                         // to
                                                                                                                                         // current?
                }
            ...
            $this->_cards[$card->getId()] = array(
                    'balance' => $balance - $balanceForFront,
