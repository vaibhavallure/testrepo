...
class Allure_CheckoutStep_Model_Checkout_Type_Onepage extends Amasty_Customerattr_Model_Rewrite_Checkout_Type_Onepage
{
	...
	public function __construct(){
		parent::__construct();
		$this->_checkoutSessionOrdered = Mage::getSingleton("checkoutstep/ordered_session");
		$this->_checkoutSessionBackordered = Mage::getSingleton("checkoutstep/backordered_session");
	}
	...
	public function saveShippingMethod($data)
	{
		...
		if(strtolower($this->getQuote()->getDeliveryMethod())==strtolower($_checkoutstepHelper::TWO_SHIP)){
			...
			if(!$this->checkQuoteItemsWithGiftCard($this->getQuoteBackordered())){
				...
				$rate = $this->getQuoteBackordered()->getShippingAddress()->getShippingRateByCode($shippingMethod2);
				if (!$rate) {
					$res = array(
							"error" => -1,
							"message" => Mage::helper("checkout")->__("Invalid shipping method.")
				...
				$this->getQuoteBackordered()->getShippingAddress()->setShippingMethod($shippingMethod2);
			...
			if(!$this->checkQuoteItemsWithGiftCard($this->getQuoteOrdered())){
				...
				$rate = $this->getQuoteOrdered()->getShippingAddress()->getShippingRateByCode($shippingMethod);
				if (!$rate) {
					$res = array(
							"error" => -1,
							"message" => Mage::helper("checkout")->__("Invalid shipping method.")
			...
			if(isset($data['mt_shipping_method']) && !empty($data['mt_shipping_method'])){
			...
			$this->getQuote()->setDeliveryMethod($_checkoutstepHelper::ONE_SHIP);
			$this->getQuote()->setOrderType($_checkoutstepHelper::SINGLE_ORDER);
	...
	public function saveDeliveryOptions000($data)
	{
		...
		if(strtolower($this->getQuote()->getDeliveryMethod())==strtolower($_checkoutstepHelper::TWO_SHIP)){
			$this->divideQuoteAsOrderAndBackorder($data);
			/*if(isset($data['delivery-shipping-method']) && !empty($data['delivery-shipping-method'])){
				if($_checkoutstepHelper::DIFFERENT==$data['delivery-shipping-method']){
					$backorder_quote = $this->getQuoteBackordered();
					$backorder_quote->getShippingAddress()->setShippingMethod($data['shipping_method']);
					$backorder_quote->collectTotals()->save();
				}else{
					$shippingMethodMain = $this->getQuote()->getShippingAddress()->getShippingMethod();
					$this->getCheckout()->setInStockOrderShippingMethod($shippingMethodMain);
				}
			}*/
		}else{
			$this->getQuote()->setOrderType($_checkoutstepHelper::SINGLE_ORDER);
	...
	public function savePayment1111($data)
	{
		...
		$quote->save();
		$_checkoutstepHelper = Mage::helper('checkoutstep');
		if(strtolower($quote->getDeliveryMethod())==strtolower($_checkoutstepHelper::TWO_SHIP)){
	...
	public function savePayment($data)
	{
		...
		if(isset($data['wholesale_pay_option']) && !empty($data['wholesale_pay_option'])){
			if($_checkoutstepHelper::PAY_AS_SHIP==$data['wholesale_pay_option']){
				if($this->isQuoteContainsBackorder() &&
						strtolower($quote->getDeliveryMethod())==strtolower($_checkoutstepHelper::ONE_SHIP))
	...
	private function applyPaymentToSecondShipment($data){
		...
		if($quoteBackOrdered){
			...
			if ($quoteBackOrdered->isVirtual()) {
				$quoteBackOrdered->getBillingAddress()->setPaymentMethod(isset($data['method']) ? $data['method'] : null);
			} else {
				$quoteBackOrdered->getShippingAddress()->setPaymentMethod(isset($data['method']) ? $data['method'] : null);
			}
	...
	private function divideQuoteAsOrderAndBackorder($data){
		...
		if($isBackorder){
			...
			$backorder_quote = Mage::getModel('sales/quote')->load($quoteMain->getId());  //unavialable qty product qoute.
			...
			if(count($backorder_quote->getAllItems()) > 0 ){
				$backorder_quote->getBillingAddress()->addData($quoteMain->getBillingAddress()->setId(null)->getData());
				$backorder_quote->getShippingAddress()->addData($quoteMain->getShippingAddress()->setId(null)->getData());
				$backorder_quote->setOrderType($_checkoutstepHelper::MULTI_BACK_ORDER);
			...
			if(count($order_quote->getAllItems()) > 0 ){
				$order_quote->getBillingAddress()->addData($quoteMain->getBillingAddress()->setId(null)->getData());
				$order_quote->getShippingAddress()->addData($quoteMain->getShippingAddress()->setId(null)->getData());
				$order_quote->setOrderType($_checkoutstepHelper::MULTI_MAIN_ORDER);
				$order_quote->save();
			...
			if(!(count($order_quote->getAllItems()) > 0  && count($backorder_quote->getAllItems()) > 0)){
				$this->getQuote()->setOrderType($_checkoutstepHelper::SINGLE_ORDER);
				$this->getQuote()->setDeliveryMethod($_checkoutstepHelper::ONE_SHIP)->save();
			}
	...
	private function divideQuoteAsOrderAndBackorder000($data){
		$isBackorder = $this->isQuoteContainsBackorder();
		$quoteMain = $this->getQuote();		//this is main quote object.
		$_checkoutstepHelper = Mage::helper('checkoutstep');
		/* $_checkoutstepHelper = Mage::helper('checkoutstep');
		...
		if(isset($orederdQuoteId) && !empty($orederdQuoteId)){
			if($this->getQuoteOrdered()->getId()!=0 && $this->getQuoteOrdered()->getId()!=$quoteMain->getId()){
		...
		if(isset($backOrderdQuoteId) && !empty($backOrderdQuoteId)){
			if($this->getQuoteBackordered()->getId()!=0 && $this->getQuoteBackordered()->getId()!=$quoteMain->getId()){
				$this->getQuoteBackordered()->setIsActive(false)->save();
			}
		...
		if($isBackorder){
			//$this->getCheckout()->setMainorder($quoteMain->getId());
			$quoteItems = $quoteMain->getAllVisibleItems();//$quoteMain->getAllItems();
			...
			if(count($backorder_quote->getAllItems()) > 0 ){
				$backorder_quote->setBillingAddress($quoteMain->getBillingAddress()->setId(null));
				$backorder_quote->setShippingAddress($quoteMain->getShippingAddress()->setId(null));
				$backorder_quote->setOrderType($_checkoutstepHelper::MULTI_BACK_ORDER);
				$backorder_quote->save();
				$backorder_quote->setIsChildOrder(1)->save();
				//Mage::log($backorder_quote->getIsChildOrder(),Zend_log::DEBUG,'abc',true);
				...
				if(isset($data['delivery-shipping-method']) && !empty($data['delivery-shipping-method'])){
					...
					->collectShippingRates()->setShippingMethod($data['shipping_method']);
						$backorder_quote->collectTotals()->save();
					/*}else{
						$backorder_quote->getShippingAddress()->setShippingMethod($backorder_quote->getShippingAddress()->getShippingMethod());
						$backorder_quote->collectTotals()->save();
					}*/
				}
			...
			if(count($order_quote->getAllItems()) > 0 ){
				$order_quote->setBillingAddress($quoteMain->getBillingAddress()->setId(null));
				$order_quote->setShippingAddress($quoteMain->getShippingAddress()->setId(null));
				$order_quote->setOrderType($_checkoutstepHelper::MULTI_MAIN_ORDER);
				$order_quote->save();
				$order_quote->setIsChildOrder(1)->save();
				$shippingMethod = $this->getCheckout()->getInStockOrderShippingMethod();
				//Mage::log("sip-00:".$shippingMethod,Zend_log::DEBUG,'abc',true);
				$order_quote->getShippingAddress()->setCollectShippingRates(true)
					->collectShippingRates()->setShippingMethod($shippingMethod);
				$order_quote->collectTotals()->save();
				//Mage::log("sip-000:".$order_quote->getShippingAddress()->getShippingMethod(),Zend_log::DEBUG,'abc',true);
				$this->getCheckoutOrdered()->setOrdered($order_quote->getId());
				$this->getCheckoutOrdered()->setQuoteId($order_quote->getId());
	...
	private function divideQuoteAsOrderAndBackorder1111(){
		$isBackorder = $this->isQuoteContainsBackorder();
		$quoteMain = $this->getQuote();		//this is main quote object.
		$_checkoutstepHelper = Mage::helper('checkoutstep');
		if($isBackorder){
			$this->getCheckout()->setBackorder(null);
			$this->getCheckout()->setOrdered(null);
			$this->getCheckout()->setMainorder($quoteMain->getId());
			...
			$backorder_quote->setId(null);
			$order_quote->setId(null);
			//$backorder_quote->assignCustomer($customer);
			//$order_quote->assignCustomer($customer);
			foreach ( $quoteItems as $item ) {
				$productInvryCount = Mage::getModel('cataloginventory/stock_item')
					->loadByProduct($item->getProduct())->getQty();
				if( $productInvryCount <= 0 ){
					$backorder_quote->addItem($item->setId(null));
				}else{Mage::log("Back-".$item->getProduct()->getId(),Zend_log::DEBUG,'abc',true);
					$order_quote->addItem($item->setId(null));
				}
			}
			if(count($backorder_quote->getAllItems()) > 0 ){
				if($session->isLoggedIn()){
					$backorder_quote->setBillingAddress($quoteMain->getBillingAddress())->save();
					$backorder_quote->setShippingAddress($quoteMain->getShippingAddress())->save();
					$backorder_quote->getBillingAddress()->setId(null);
					$backorder_quote->getShippingAddress()->setId(null);
				} else {
					$backorder_quote->setBillingAddress($quoteMain->getBillingAddress())->save();
					$backorder_quote->setShippingAddress($quoteMain->getShippingAddress())->save();
					$backorder_quote->getBillingAddress()->setId(null);
				...
				$backorder_quote->getPayment()->setId(null);
				if ($quoteMain->isVirtual()) {
					$backorder_quote->getBillingAddress()->setPaymentMethod($quoteMain->getPayment()->getMethod());
				} else {
					$backorder_quote->getShippingAddress()->setPaymentMethod($quoteMain->getPayment()->getMethod());
				}
				
				$backorder_quote->getShippingAddress()->setCollectShippingRates(true)
					->collectShippingRates()
					->setShippingMethod($backorder_quote->getShippingAddress()->getShippingMethod());
				
				$backorder_quote->getPayment()->setMethod($quoteMain->getPayment()->getMethod());
				$backorder_quote->getPayment()->importData(array('method' => $quoteMain->getPayment()->getMethod()))->save();
				$backorder_quote->setTotalsCollectedFlag(false)->collectTotals()->save();
				$this->getCheckout()->setBackorder($backorder_quote->getId());
				$this->getCheckoutBackordered()->setQuoteId($backorder_quote->getId());
			}
			
			if(count($order_quote->getAllItems()) > 0 ){
				if($session->isLoggedIn()){
					$order_quote->setBillingAddress($quoteMain->getBillingAddress())->save();
					$order_quote->setShippingAddress($quoteMain->getShippingAddress())->save();
					$order_quote->getShippingAddress()->setId(null);
					$order_quote->getBillingAddress()->setId(null);
				} else {
					$order_quote->setBillingAddress($quoteMain->getBillingAddress())->save();
					$order_quote->setShippingAddress($quoteMain->getShippingAddress())->save();
					$order_quote->getBillingAddress()->setId(null);
				...
				$order_quote->getPayment()->setId(null);
				if ($quoteMain->isVirtual()) {
					$order_quote->getBillingAddress()->setPaymentMethod($quoteMain->getPayment()->getMethod());
				} else {
					$order_quote->getShippingAddress()->setPaymentMethod($quoteMain->getPayment()->getMethod());
				}
				
				$order_quote->getShippingAddress()->setCollectShippingRates(true)
					->collectShippingRates()
					->setShippingMethod($order_quote->getShippingAddress()->getShippingMethod());
				
				$order_quote->getPayment()->setMethod($quoteMain->getPayment()->getMethod());
				$order_quote->getPayment()->importData(array('method' => $quoteMain->getPayment()->getMethod()))->save();
				$order_quote->setTotalsCollectedFlag(false)->collectTotals()->save();
				$this->getCheckout()->setOrdered($order_quote->getId());
				$this->getCheckoutOrdered()->setQuoteId($order_quote->getId());
			}
			if(!(count($order_quote->getAllItems()) > 0  && count($backorder_quote->getAllItems()) > 0)){
				$this->getQuote()->setDeliveryMethod($_checkoutstepHelper::ONE_SHIP)->save();
	...
	public function changeCustsomQuoteStatus(){
		...
		if(isset($backOrderdQuoteId) && !empty($backOrderdQuoteId)){
			if($this->getQuoteBackordered()->getId()!=0 && $this->getQuoteBackordered()->getId()!=$this->getQuote()->getId()){
				$this->getQuoteBackordered()->setIsActive(false)->save();
			}
			$this->getCheckoutBackordered()->setBackorder(null);
		}
	...
	{
		$_checkoutstepHelper = Mage::helper('checkoutstep');
		if(strtolower($this->getQuote()->getDeliveryMethod())==strtolower($_checkoutstepHelper::ONE_SHIP)){
			...
			$quote = $this->getQuoteBackordered();
			Mage::log("\n********************** OUT OF STOCK START **************************",Zend_Log::DEBUG,'multiorder.log',true);
			...
			Mage::log(json_encode($quote->getShippingAddress()->getShippingMethod()),Zend_log::DEBUG,'multiorder.log',true);
			Mage::log("\n----------Payment Data-------",Zend_log::DEBUG,'multiorder.log',true);
			Mage::log(json_encode($data),Zend_log::DEBUG,'multiorder.log',true);
			Mage::log("\n********************** OUT OF STOCK END **************************",Zend_Log::DEBUG,'multiorder.log',true);
			if(!$quote->getIsCheckoutCart()){
				...
				$this->getCheckoutBackordered()->setSecondLastQuoteId($quote->getId())
			...
			$quote = $this->getQuoteOrdered();
			Mage::log("\n********************** IN STOCK START **************************",Zend_Log::DEBUG,'multiorder.log',true);
			Mage::log("\n----------Quote Data-------",Zend_log::DEBUG,'multiorder.log',true);
			Mage::log(json_encode($quote->getData()),Zend_log::DEBUG,'multiorder.log',true);
			Mage::log("\n----------Shipping Address-------",Zend_log::DEBUG,'multiorder.log',true);
			Mage::log(json_encode($quote->getShippingAddress()->getData()),Zend_log::DEBUG,'multiorder.log',true);
			...
			Mage::log("\n----------Payment Data-------",Zend_log::DEBUG,'multiorder.log',true);
			Mage::log(json_encode($data),Zend_log::DEBUG,'multiorder.log',true);
			Mage::log("\n********************** IN STOCK END **************************",Zend_Log::DEBUG,'multiorder.log',true);
			if(!$quote->getIsCheckoutCart()){
				...
				$this->getCheckoutOrdered()->setLastQuoteId($quote->getId())
    ...
    public function saveCustomOrder($data)
    {
    	...
    	if(strtolower($this->getQuote()->getDeliveryMethod())==strtolower($_checkoutstepHelper::ONE_SHIP)
    		...
    		if(!$quote->getIsCheckoutCart()){
	    		...
	    		//update increment id for backorder.
	    		/* if($secondOrder->getId()!=0){
	    			$secondOrderObj = Mage::getModel('sales/order')->load($secondOrder->getId());
	    			$newIncrementId = $firstOrderIncrementId."-B";
	    			if(!empty($firstOrderIncrementId)){
	    				$secondOrderObj->setIncrementId($newIncrementId);
	    				$secondOrderObj->save();
	    			}
	    		} */
