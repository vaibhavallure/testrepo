...
class Allure_CheckoutStep_Block_Checkout_Onepage_Shipping_Method_Available extends Mage_Checkout_Block_Onepage_Shipping_Method_Available
{
    ...
    public function getShippingRatesForTwoShipInStockProducts(){
    	...
    	if(empty($this->_rates_twoship_instock)){
    		...
    		$this->getAddressForTwoShipInStockProducts()->collectShippingRates()->save();
    ...
    public function getShippingRatesForTwoShipOutOfStockProducts(){
    	if(empty($this->_rates_twoship_outofstock)){
    		...
    		$this->getAddressForTwoShipOutOfStockProducts()->collectShippingRates()->save();
    ...
    public function checkGiftCardIsPresentOutofStockOrder(){
    	$quote = Mage::getSingleton("checkoutstep/backordered_session")->getQuote();
    	return  $this->checkQuoteItemsWithGiftCard($quote);
    }
    ...
    public function checkGiftCardInQuotes(){
    	...
    	$status = array('both'=>$isGiftCardWithTwoQuotes,
    			'first_ship_method'=>$isGiftCardInStockQuote,'second_ship_method'=>$isGiftCardOutOfStockQuote);
    	return $status;
    }
    public function getOrderTotal(){
    	...
    	if($quote['delivery_method']=='two_ship'){
    		$quote1 = Mage::getSingleton("checkoutstep/ordered_session")->getQuote()->getData();
    		$quote2 = Mage::getSingleton("checkoutstep/backordered_session")->getQuote()->getData();
    		$type['two_ship']=array('in_stock_total'=>$quote1['base_subtotal'],
    				'out_stock_total'=>$quote2['base_subtotal']);
    	}
    	else{
    		$type['one_ship']=array('total'=>$quote['base_subtotal']);
    	}
