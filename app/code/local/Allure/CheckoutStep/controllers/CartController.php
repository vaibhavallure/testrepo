...
class Allure_CheckoutStep_CartController extends Ecp_Shoppingcart_CartController//Webtex_Giftcards_CartController
{
    private function _setShippingInfo(array $info) 
    {
        ...
        if (!empty($code)) {
        	...
        	$shippingAddress
        		...
        		->save();
        	$taxes = Mage::getSingleton('sales/quote_address_total_tax',array('store' => $this->_getQuote()->getStore()));
    ...
    public function couponPostAction()
    {
        ...
        try {
            ...
            $this->_getQuote()->setTotalsCollectedFlag(false)->setCouponCode(strlen($couponCode) ? $couponCode : '')
                ->collectTotals()
                ->save();
            
                //coupon code apply to two shipment quote's.
                $_checkoutstepHelper = Mage::helper('checkoutstep');
                if(strtolower($this->_getQuote()->getDeliveryMethod())==
                		strtolower($_checkoutstepHelper::TWO_SHIP)){
                	
                	$model = Mage::getModel('checkout/type_onepage');		
                	$model->getQuoteOrdered()->getShippingAddress()->setCollectShippingRates(true);
                	$model->getQuoteOrdered()->setTotalsCollectedFlag(false)->setCouponCode(strlen($couponCode) ? $couponCode : '')
                		->collectTotals()->save();
                	
                	$model->getQuoteBackordered()->getShippingAddress()->setCollectShippingRates(true);
                	$model->getQuoteBackordered()->setTotalsCollectedFlag(false)->setCouponCode(strlen($couponCode) ? $couponCode : '')
                		->collectTotals()->save();
