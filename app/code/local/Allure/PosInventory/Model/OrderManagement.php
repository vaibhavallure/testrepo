...
class Allure_PosInventory_Model_OrderManagement extends  Ebizmarts_BakerlooRestful_Model_OrderManagement
{
        ...
        try {
            ...
            foreach ($order->getAllItems() as $orderItem){
                ...
                if(/* $orderItem->getData('base_price') */$baseRowTotalInclTax < 0 && $baseRowTotalInclTax!=null){
                    ...
                    if($baseRowTotalInclTax < 0){
                        $refundedAmt = $refundedAmt + ($baseRowTotalInclTax*(-1));
                    }else {
            ...
            if($refundedAmt > 0){
                ...
                $subtotal = $nonRefundedAmt - $refundedAmt ;//$quote->getSubtotal() - $refundedAmt;
                $subtotal       = $subtotal;//$quote->getSubtotal();
                $baseSubtotal   = $subtotal;//$quote->getBaseSubtotal()  ;
                $grandTotal     = $subtotal;//$quote->getGrandTotal() ;
                $baseGrandTotal = $subtotal;//$quote->getBaseGrandTotal() ;
                if($isDiscount){
                    $subtotal       += $extraDiscount;
                    $baseSubtotal   += $extraDiscount;
                ...
                $order->setTotalPaid($totalPaidAmt);
