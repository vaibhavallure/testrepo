...
class Allure_InstaCatalog_FeedController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loadMoreAjaxAction(){
    	...
    	if($request['page'])
    		...
    		$strHtml = "";
    		foreach ($feeds as $_post) {
	    		$media_id = $_post->getId();//getMediaId();
	    		$username = $_post->getUsername();
	    		$class_css = "post_item_without_link";
	    		/* if (strpos($username,'instagram') !== false)  */
	    		if($_post->getProductCount()>0)
	    			...
	    			$strHtml .= '<div id="fs-post-'.$media_id.'" class="fs-timeline-entry" onclick="instagramView.show(this);" media-id="'.$media_id.'"';
	 	    		...
	 	    		$strHtml .= '<i media-id="'.$media_id.'" class="fs-icon fs-fa-instagram"></i>';
	 	    		$strHtml .= '</div>';
	 	    		$strHtml .= '<span id="shop-'.$media_id.'" class="instagram_shop_button '.$class_css.'" target="_blank">';
	 	    		$strHtml .= '<span class="shop-it-button">SHOP IT</span>';
	 	    		$strHtml .= '</span>';
	 	    		$strHtml .= '<div style="display: none;" id="details-insta-'.$media_id.'"';
	 	    		...
	 	    		$strHtml .= '<div id="insta-caption-'.$media_id.'" style="display: none;">'.$instaCaption.'</div>';
	 	    		$strHtml .= '<div id="insta-product-mark-'.$media_id.'" style="display: none;">'.$points.'</div>';
	 	    		$strHtml .= '<div id="insta-product-details-'.$media_id.'" style="display: none;">'.$productsLinks.'</div>';
	 	    		$strHtml .= '</div>';
	 	    		$strHtml .= '</div>';
	 	    		$strHtml .= '</div>';
	 	    		$strHtml .= '</div>';
