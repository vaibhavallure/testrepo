    ...
    public function loadAjaxAction(){
        ...
        if ($feeds)
            foreach ($feeds as $_post) {
                ...
                $str .= '<div>';
                $str .= '<i onclick="instagramView.show(this);" media-id="'.$mediaId.'" ></i>';
                $str .= '</div>';
    ...
    public function loadMoreAjaxAction(){
        ...
        if ($feeds)
            foreach ($feeds as $_post) {
                ...
                $insta_logo = Mage::getDesign()->getSkinUrl('images/social/instagram_new.png');
                $strHtml .= '<i  media-id="'.$mediaId.'" > <img width="21px" src="'.$insta_logo.'" ></i>';
                $strHtml .= '</div>';
