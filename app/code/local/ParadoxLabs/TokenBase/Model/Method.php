...
class ParadoxLabs_TokenBase_Model_Method extends Mage_Payment_Model_Method_Cc
{
	...
	public function assignData( $data )
	{
		...
		if( $data->hasCardId() && $data->getCardId() != '' ) {
			...
			$this->getInfoInstance()->setData('tokenbase_id', null);
		}
	...
	protected function _loadOrCreateCard( Varien_Object $payment )
	{
		...
		if (empty($payment->hasTokenbaseId())){
		    $parentIncrementId=str_replace('-B', '', $payment->getOrder()->getIncrementId());
		    $parentOrderId=Mage::getModel('sales/order')->loadByIncrementId($parentIncrementId)->getId();
		    $parentPayment=Mage::getModel('sales/order_payment')->load($parentOrderId,'parent_id');
		    if(!empty($parentPayment->getTokenbaseId()))
		        $payment->setTokenbaseId($parentPayment->getTokenbaseId());
		}
