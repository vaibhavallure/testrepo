...
class Ecp_ReportToEmail_Model_Observer
{
	...
	//	Mage::log($emails);
        ...
        if ($time != (int)$curTime->format("H")) return;
        ...
        $collection->addFieldToFilter($report_type, array('from'=>$fromDate, 'to'=>$toDate));
        $collection->getSelect()->order('period DESC');
        $rows = array();
        $_headers = array();
		//Mage::log(count($collection));
        $needColumns = array('period','orders_count','total_income_amount');
...
//	Mage::log($mailbody);
	...
	$sender=Mage::getStoreConfig('trans_email/ident_general/email');
        ...
        $mail->setBodyHtml($mailbody)
            ->setSubject('Maria Tash: Daily Report')
            ->addTo($emails)
