...
class Ecp_Category_Model_Resource_Layer_Filter_Price extends Mage_Catalog_Model_Resource_Layer_Filter_Price
{
    /**
     * Minimal possible price
     */
    const MIN_POSSIBLE_PRICE = .00;
    ...
    public function applyPriceRange($filter)
    {
        ...
        if ($from !== '') {
            $from = round($this->_getComparingValue($from, $filter),0,PHP_ROUND_HALF_UP);
            $select->where($priceExpr . ' >= ' . $from);
        }
        if ($to !== '') {
            $to = round($this->_getComparingValue($to, $filter),0,PHP_ROUND_HALF_DOWN);
