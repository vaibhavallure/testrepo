 ...
 */
class FME_Restrictcustomergroup_Adminhtml_AbstractblockController
    ...
    public function newConditionHtmlAction() {
        ...
        if ($this->_validateFormKey()) {
            ...
            $typeArr = $this->getRequest()->getParam('type') ? $this->getRequest()->getParam('type') : 'restrictcustomergroup-rule_condition_combine';
            $typeArr = explode('|', str_replace('-', '/', $typeArr));
            $type = $typeArr[0];
            $prefix = ($this->getRequest()->getParam('prefix')) ? $this->getRequest()->getParam('prefix') : 'conditions';
            $rule = ($this->getRequest()->getParam('rule')) ? base64_decode($this->getRequest()->getParam('rule')) : 'restrictcustomergroup/restrictcustomergroup';
    ...
    protected function _setTitle($action) {
        ...
        if (method_exists($this, '_title')) {
            $this->_title($this->__('Restrict Customer Group Rules'))->_title($this->__($action));
        }
        return $this;
    }
