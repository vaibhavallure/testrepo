...
abstract class Amazon_Payments_Controller_Checkout extends Mage_Checkout_Controller_Action
{
    ...
    public function preDispatch()
    {
        ...
        if($token){
            $_amazonLogin1 = Mage::getModel('amazon_payments/customer');
            $amazonProfile = $_amazonLogin1->getAmazonProfile($token);
            if ($amazonProfile && isset($amazonProfile['email'])) {
                ...
                Mage::getSingleton('checkout/session')->getQuote()->setEmail($amazonProfile['email']);
            }
        }
