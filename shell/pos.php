...
class Ebizmarts_PosShell_Pos extends Mage_Shell_Abstract
{
    ...
    public function tryAgain()
    {
        ...
        if ($orderId = $this->getArg('order-id')) {
            $bakerlooOrder = Mage::getModel('bakerloo_restful/order')->load($orderId);

            if (!$bakerlooOrder->getId()) {
                $this->trace("++++++++++ Can't find order " . $orderId . ": order doesn't exist. ++++++++++\n\n");
                return;
            }

            $this->placeOrder($bakerlooOrder);
            return;
        } else {
            $orders = Mage::getModel('bakerloo_restful/order')->getCollection()->addFieldToFilter('order_id', array('eq' => 0))->getItems();
    ...
    private function placeOrder($bakerlooOrder)
    {

        $json = json_decode($bakerlooOrder->getJsonPayload());
        $headers = $bakerlooOrder->getHttpHeaders();

        if (!$json or !$headers) {
            $this->trace("++++++++++ Can't process order " . $bakerlooOrder->getId() . ": missing data. ++++++++++\n\n");
            return;
        }

        if ($bakerlooOrder->getOrderId()) {
            $this->trace("++++++++++ Can't process order " . $bakerlooOrder->getId() . ": order already submitted. ++++++++++\n\n");
            return;
        }

        $this->trace("++++++++++ Submitting order " . $bakerlooOrder->getId() . " ++++++++++\n\n", true);

        $response = Mage::helper('bakerloo_restful/http')->POST($bakerlooOrder->getRequestUrl(), json_encode($json), $headers);

        $objResponse = json_decode($response, true);

        if (!is_array($objResponse)) {
            $this->trace("++++++++++ Error processing order " . $bakerlooOrder->getId() . ": " . $objResponse . "\n\n");
            return;
        } elseif (isset($objResponse['error_message'])) {
            $this->trace("++++++++++ Error processing order " . $bakerlooOrder->getId() . ": " . $objResponse['error_message'] . "\n\n");
            return;
        }

        $this->trace("++++++++++ Order " . $bakerlooOrder->getId() . " OK. ++++++++++\n\n", true);
    }
    ...
    public function generateFiles($store)
    {
        ...
        if ($this->getArg('all')) {
            $nrOfResources = count($this->_resources);
