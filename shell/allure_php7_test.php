 ...
 * Allure d.o.o.
 *
 * Test if Allure_PHP is installed and integrated into Magento correctly.
 *
 * @author Ivan Čurdinjaković <ivan.curdinjakovic@inchoo.net>
 */
class Allure_PHP7_Test extends Mage_Shell_Abstract
{
    ...
    {
        return $this->coreHelper->isModuleEnabled('Allure_PHP7');
    }

    /**
     * Check models class name, or parent class name.
     *
     * Parent class name - in case some other rewrite inherits from Allure_PHP7, fixing a rewrite conflict.
     *
     * @param object $model
    ...
    /**
     * Checks if a class has ALLURE_PHP7 const, proving it is correctly overriden
     * @param string $class
     * @return bool
     */
    private function checkOverride(string $class): bool
    {
        return defined("$class::ALLURE_PHP7");
    }

    private function isCoreHelperRewritten(): bool
    {
        return $this->checkRewrite($this->coreHelper, 'Allure_PHP7_Helper_Data');
    }

    private function isCoreLayoutRewritten(): bool
    {
        return $this->checkRewrite(Mage::getModel('core/layout'), 'Allure_PHP7_Model_Layout');
    }
    ...
    private function isImportEntityProductRewritten(): bool
    {
        return $this->checkRewrite(
            ...
            'Allure_PHP7_Model_Import_Entity_Product'
        );
    }
    ...
    private function isExportEntityConfigurableRewritten(): bool
    {
        return $this->checkRewrite(
            ...
            'Allure_PHP7_Model_Export_Entity_Product_Type_Configurable'
        );
    }
    ...
    private function isExportEntityGroupedRewritten(): bool
    {
        return $this->checkRewrite(
            ...
            'Allure_PHP7_Model_Export_Entity_Product_Type_Grouped'
        );
    }
    ...
    private function isExportEntitySimpleRewritten(): bool
    {
        return $this->checkRewrite(
            ...
            'Allure_PHP7_Model_Export_Entity_Product_Type_Simple'
        );
    }
    ...
    private function isExportEntityCustomerRewritten(): bool
    {
        return $this->checkRewrite(
            ...
            'Allure_PHP7_Model_Export_Entity_Customer'
        );
    }
    ...
    private function checkExtensionAndCoreVersions(): int
    {
        ...
        $extensionVersion = Mage::getConfig()->getNode('modules/Allure_PHP7/version');
        $extensionMajorVersion = (int)substr($extensionVersion, 0, 1);
        $edition = Mage::getEdition();

        echo "$this->testsRun: ";
        echo "Allure_PHP7 version is: $extensionVersion, Magento core version is: $edition $mCoreVersion - ";
        ...
        if ($edition === 'Enterprise') {
            if ($extensionMajorVersion == 1) {
                if (version_compare($mCoreVersion, '1.14.2.0') >= 0 && version_compare($mCoreVersion, '1.14.3.0') < 0){
                    ...
                    exit("You need version 2.* of Allure_PHP7 extension for this core!\n");
                } else {
                    echo "Magento core version is too low. This MAY or MAY NOT work!\n";
                }
            } else {
                if (version_compare($mCoreVersion, '1.14.2.0') >= 0 && version_compare($mCoreVersion, '1.14.3.0') < 0){
                    exit("You need version 1.* of Allure_PHP7 extension for this core!\n");
                } elseif (version_compare($mCoreVersion, '1.14.3.0') >= 0){
                    $this->testsSuccessful++;
                    echo "Ok.\n";
                } else {
    ...
    public function run()
    {
        ...
        $this->doHaltingTest(
            [$this, 'isEnabled'],
            'Allure_PHP7 module is enabled.',
            'Allure_PHP7 module is NOT enabled!'
        );
