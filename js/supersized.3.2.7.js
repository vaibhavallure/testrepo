    ...
    $.supersized = function(options){
    	...
    	base._start = function(){
			...
			if (base.options.slides.length > 1){
				...
				imgNext.appendTo(slideNext).wrap(anchor).parent().parent().addClass('image-loading');
		...
		base.nextSlide = function(){
			...
			if (!$(targetList).html()){
				...
				if (imageLink != "") {
					anchor = '<a '+imageLink+linkTarget+'></a>'
				}
				img.appendTo(targetList).wrap(anchor).parent().parent().addClass('image-loading').css('visibility','hidden');
