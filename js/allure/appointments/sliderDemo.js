...
(function ($, undefined) {
	...
	$.widget("ui.sliderDemo", {
		...
		_create: function(){
			...
			for (var int = 1; int <= 42; int++) {
				
				var demo = $('#time_slot_'+int);
				demo.click($.proxy(this._changingValues, this));
							
			}
			
			//this._setLabelsOption("valueLabels","hide");
		...
		_createArrows: function(){
			...
			if (!this.options.arrows){
				...
				this.element.bind("valuesChanging valuesChanged userValuesChanged", $.proxy(this._returnValues, this));
			}
		},
		...
		_changed: function(isAutomatic){
			...
			if (this._updateValues() || this._valuesChanged){
				...
				var demo = $$('#time_slot_'+this._values.min);
				
				if(demo[0]){
				var minValue= demo[0].getAttribute('data-start_time_slot');
				
				var maxValue= demo[0].getAttribute('data-end_time_slot');
				
				var minSelect = $$('.ui-rangeSlider-label-value');

				minSelect[0].textContent = minValue ;
				
				minSelect[1].textContent = maxValue;
				
				//this._values.min = maxValue;
				//this._values.max = minValue;
		...
		_updateValues: function(){
			...
			//this._setBoundsOption("bounds", this._values);
			...
			var left = $('.ui-rangeSlider-leftLabel');
			//minSelect[0].css("position", "absolute")
			left.css("margin-right: 5px");
			
			var right = $('.ui-rangeSlider-rightLabel');
			var dddd = right.css("right");
			var newValue = parseInt(dddd) - 35;
			
			right.css("right",newValue);
		...
		_changingValues: function(e,data){
			
			var minValue = e.currentTarget.getAttribute('data-start_time_slot');
			
			var maxValue= e.currentTarget.getAttribute('data-end_time_slot');
			
			var pos = e.currentTarget.getAttribute('data-pos');
			
			this._values.min = parseInt(pos);
			this._values.max = parseInt(pos) + 2; 
			this._trigger("valuesChanging");
			//this._setLabelsOption("valueLabels","show");
		...
		_returnValues: function(data){
			...
			if(demo[0]){
				...
				demo.css("background-color","palegoldenrod");
			...
			$("#starttime_hidden").val(minValue);
			$("#endtime_hidden").val(maxValue);
			...
			//minSelect[1].css("position", "absolute")
			}
