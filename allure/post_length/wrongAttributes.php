    ...
    foreach ($productCollection  as $product) {
        ...
        if (count($oldItemSku) == 3) {
            ...
            if ($newItemId) {
                ...
                if ($product->getAttributeSetId() != $newProduct->getAttributeSetId()) {
                    $count=$count+1;
                    print ($oldItem."::".$newItem."-------::NEW SKU ATTRIBUTE SET MISMATCH::".getAttributeSetName($product->getAttributeSetId())."::".getAttributeSetName($newProduct->getAttributeSetId())."\n");
                    Mage::log("Count::".$count." ".$oldItem."::".$newItem."-------::NEW SKU ATTRIBUTE SET MISMATCH::".getAttributeSetName($product->getAttributeSetId())."::".getAttributeSetName($newProduct->getAttributeSetId())."\n",Zend_Log::DEBUG,'wrong_attribute.log',true);
                    $query = "UPDATE `catalog_product_entity` SET `attribute_set_id`='{$product->getAttributeSetId()}' WHERE `entity_id`='{$newProduct->getId()}';";
                    $writeAdapter->query($query);
                }
                
                if (!$parentItemId) {
                    print ($oldItem."::".$parentItem."-----::PARENT SKU DOES NOT EXISTS\n");
                    continue;
                }
                
                $parentProduct = Mage::getModel('catalog/product')->load($parentItemId);
                
                if ($product->getAttributeSetId() != $parentProduct->getAttributeSetId()) {
                    $count=$count+1;
                    print ($oldItem."::".$parentItem."----------::PARENT SKU ATTRIBUTE SET MISMATCH::".getAttributeSetName($product->getAttributeSetId())."::".getAttributeSetName($parentProduct->getAttributeSetId())."\n");
                    Mage::log("Count::".$count." ".$oldItem."::".$parentItem."----------::PARENT SKU ATTRIBUTE SET MISMATCH::".getAttributeSetName($product->getAttributeSetId())."::".getAttributeSetName($parentProduct->getAttributeSetId())."\n",Zend_log::DEBUG,'wrong_attribute.log',true);
                    $query = "UPDATE `catalog_product_entity` SET `attribute_set_id`='{$product->getAttributeSetId()}' WHERE `entity_id`='{$parentProduct->getId()}';";
                    $writeAdapter->query($query);
                }
            } else {
                print ($oldItem."::".$newItem."::-------NEW SKU DOES NOT EXISTS\n");
            }
        }
...
function getAttributeSetName($attributeId) {
    static $attributeSetNames = array();
    if (!isset($attributeSetNames[$attributeId])) {
        $attributeSetNames[$attributeId] = Mage::getModel('eav/entity_attribute_set')->load($attributeId)->getAttributeSetName();
    }
    
    return $attributeSetNames[$attributeId];
}