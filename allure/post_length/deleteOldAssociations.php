...
$fixedItems = array("CCLV7DPS_C");//CSQGPS_E XCLVD
...
foreach ($fixedItems as $fixedSku) {
	Mage::log('Parsing Fixed SKU:: '.$fixedSku, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
	...
	foreach ($productCollection  as $product) {
		...
		Mage::log('Old SKU :: '.$oldItem, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
		...
		if (count($oldItemSku) == 3) {
			...
			Mage::log('New SKU :: '.$newItem, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
			Mage::log('Parent SKU :: '.$parentItem, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
			var_dump("New SKU: ".$newItem);
			var_dump("Parent SKU: ".$parentItem);
			
			Mage::log('Post Length :: '.$post_length, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
			...
			Mage::log('Original Id :: '.$oldItemId, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
			Mage::log('New Id :: '.$newItemId, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
			Mage::log('Parent Id :: '.$parentItemId, Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
			...
			if ($newItemId) {
        			...
        			if (!empty($oldItemId) && (!isset($oldAssociations[$parentItemId]) || !isset($oldAssociations[$parentItemId][$oldItemId]))) {
        			    
        			    if (!isset($oldAssociations[$parentItemId])) {
        			        $oldAssociations[$parentItemId] = array();
        			    }
        			    
        			    Mage::log('OLD ITEM FOUND !!', Zend_Log::DEBUG, 'post_length_migrations_parsing.log', true);
        			    
        			    $oldAssociations[$parentItemId][$oldItemId] = $oldItemId;
        			    
        			}
        
        			if (!empty($newItemId) && (!isset($newAssociations[$parentItemId]) || !isset($newAssociations[$parentItemId][$newItemId]))) {
        
        			    if (!isset($newAssociations[$parentItemId])) {
        			        $newAssociations[$parentItemId] = array();
        				}
        			
        				Mage::log('NEW ITEM FOUND !!', Zend_Log::DEBUG, 'post_length_associations_parsing.log', true);
        				
        				$newAssociations[$parentItemId][$newItemId] = $newItemId;
...
foreach ($newAssociations as $product_id => $associations) {
	...
	if (empty($sku) || in_array($sku, $skippedSkus)) {
		Mage::log('Skipping Custom Options for SKU:: '.$sku, Zend_Log::DEBUG, 'post_length_associations_processing.log', true);
		var_dump('Skipping Custom Options for SKU:: '.$sku);
		continue;
	}

	Mage::log('Updating Associations for SKU:: '.$sku, Zend_Log::DEBUG, 'post_length_associations_processing.log', true);
	var_dump('Updating Associations for SKU:: '.$sku);

	if (empty($sku) || empty($associations)) {
		continue;
	}
	
	$oldAssociationsList = implode('|', $oldAssociations[$product_id]);
	$associationsList = implode('|', $associations);
	
	Mage::log('Old Associations:: '.$oldAssociationsList, Zend_Log::DEBUG, 'post_length_associations_processing.log', true);
	var_dump('Old Associations:: '.$oldAssociationsList);
	
	Mage::log('New Associations:: '.$associationsList, Zend_Log::DEBUG, 'post_length_associations_processing.log', true);
	var_dump('New Associations:: '.$associationsList);
    ...
    try {
		...
		Mage::log('Failed Updating Associations for SKU:: '.$sku, Zend_Log::DEBUG, 'post_length_associations_processing.log', true);
		var_dump('Failed Updating Associations for SKU:: '.$sku);
	}
	
	unset($_product);
	
	if (($recordIndex % 300) == 0) {
	    $writeAdapter->commit();
	    $writeAdapter->beginTransaction();
	}
}
