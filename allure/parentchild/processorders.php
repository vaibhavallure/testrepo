...
$range = $_GET['range'];  //all or single
if(empty($range)){
    die('Please mension range as single or all');
}
$store = $_GET['store'];
...
if($range=='all' && (empty($from)||empty($to))){
    die('please add from and to date range');
}

$order= $_GET['order'];

if($range=='single' && empty($order)){
...
if($range=='single' && !empty($order)){
    $orders->addAttributeToFilter('increment_id', array('eq' => array($order)));
}
$orders->addAttributeToFilter('status', array('processing','pending'));



foreach ($orders as $order){
   $isLenthItemAvailbale=false;
   foreach ($order->getAllVisibleItems() as $item){
            
            if(preg_match("/MM/", $item->getSku()) ||preg_match("/mm/", $item->getSku())){
               
                $productOldId = Mage::getModel("catalog/product")->getIdBySku($item->getSku());
                $productOldArray = explode('|', $item->getSku());
                
                if (count($productOldArray) > 2 && in_array($productOldArray[0], $fixedItems)){
                    array_pop($productOldArray);
                    $withoutPostlengthSku = implode('|', $productOldArray);
                    $productId= Mage::getModel('catalog/product')->getIdBySku($withoutPostlengthSku);
                    $simpleProduct=Mage::getModel('catalog/product')->load($productId);
                    
                    if(isset($productId) && !empty($productId)){
                        $isLenthItemAvailbale=true;
                       // $item->setName($simpleProduct->getName());
                        $item->setSku($simpleProduct->getSku());
                        $cpid=$simpleProduct->getId();
                        $product_options = unserialize($item['product_options']);
                        $oldObj=$product_options['info_buyRequest'];
                        $superAttribute=$oldObj['super_attribute'];
                        ...
                        $oldObj['super_attribute']=$superAttribute;
                        
                        $product_options['info_buyRequest']=($oldObj);
                        $attributesInfo=$product_options['attributes_info'];
                        foreach ($attributesInfo as $key=>$value){
                            if($value['label']=='Post Length'){
                                unset($attributesInfo[$key]);
                            }
                        
                        }
                        $product_options['attributes_info']=($attributesInfo);
                        //$product_options['simple_name']=$simpleProduct->getName();
                        $product_options['simple_sku']=$simpleProduct->getSku();
            ...
            }   //End of sku check contain length
        }  //End of Foreach for item
        if($isLenthItemAvailbale){
            Mage::log("Item processed for Order::".$order->getIncrementId(),Zend_log::DEBUG,'parentchild_processorders.log',true);
            
        }
        
}  //End of foreach for order
