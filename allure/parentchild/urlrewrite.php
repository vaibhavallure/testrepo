...
/* $product=Mage::getModel('catalog/product')->load(23118);
...
->addAttributeToFilter('sku', array('like' => '%CFL55CZ%'))
        ...
        if (count($productOldArray) > 2 && in_array($configSku, $fixedItems)){
            ...
            if($withoutPostlengthProduct->getId()){
                ...
                try {
                    ...
                    $productOldUrlArray = explode('|', $url);
                    $endWord=end($productOldUrlArray);
                    if(strtolower($endWord)=='old')
                        array_pop($productOldUrlArray);
                    $oldUrl=implode('|', $productOldUrlArray);
                    ...
                    ->setRequestPath($oldUrl.'.html')
                    ->save();
                    
                    echo $product->getSku();
                    echo "<br>";
                    Mage::log($count."-product Id:".$product->getId(),Zend_log::DEBUG,'parentchild_url.log',true);
                    Mage::log("Target Url:".$configProduct->getUrlKey().'.html',Zend_log::DEBUG,'parentchild_url.log',true);
                    Mage::log("Request Url:".$oldUrl.'.html',Zend_log::DEBUG,'parentchild_url.log',true);
                    
                    
                } catch (Exception $e) {
                    //var_dump($e->getMessage());
                    
                    Mage::log($count."-product Id:".$product->getId(),Zend_log::DEBUG,'parentchild_url_fail.log',true);
                    Mage::log("Request Url:".$oldUrl.'.html',Zend_log::DEBUG,'parentchild_url_fail.log',true);
