...
foreach ($collection as $product)
{
    if(preg_match("/MM/", $product->getSku()) ||preg_match("/mm/", $product->getSku())){
        ...
        $productSkuArray = explode('|', $product->getSku());
        ...
        if (count($productSkuArray) > 2 && in_array($configSku, $fixedItems)){
            array_pop($productSkuArray);
            $withoutPostlengthSku = implode('|', $productSkuArray);
            ...
            if($withoutPostlengthProduct->getId()){
                ...
                try {
                    ...
                    Mage::log("Request Url:".$withoutOldWordUrl.'.html',Zend_log::DEBUG,'parentchild_url.log',true);
                    Mage::log("Product Url:".$url.'.html',Zend_log::DEBUG,'parentchild_url.log',true);
                    $countsucess++;
                    
                    
                } catch (Exception $e) {
                    
                    Mage::log($countfail."-product Id:".$product->getId(),Zend_log::DEBUG,'parentchild_url_fail.log',true);
                    Mage::log("Request Url:".$url.'.html',Zend_log::DEBUG,'parentchild_url_fail.log',true);
                    $countfail++;
