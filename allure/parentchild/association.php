...
foreach ($fixedItems as $fixedSku) {
   ...
   /*      print_r($childProducts[0]->getId());
        die; */
    
    $newAssoProductsArray=array();
    foreach ($childProducts as $child){
            
            $childSku=$child->getSku();
            $newAssoProductsArray[]=$child->getId();
            ...
            if(preg_match("/MM/", $childSku) || preg_match("/mm/", $childSku)) {   //Check contains length
                ...
                if (count($childSkuArray) > 2) {
                    ...
                    //Mage::log('Pop Array :: '.$withoutPostlength, Zend_Log::DEBUG, 'parent_child_association.log', true);
                    $withoutPostlengthSku = implode('|', $childSkuArray);
                    
                    Mage::log('Simple SKU without Length :: '.$withoutPostlengthSku, Zend_Log::DEBUG, 'parent_child_association.log', true);
                    $productIdWithoutLength = Mage::getModel('catalog/product')->getIdBySku($withoutPostlengthSku);
                    $productNew=Mage::getModel('catalog/product')->load($productIdWithoutLength);
                    ...
                    if(isset($productIdWithoutLength) && !empty($productIdWithoutLength) && $productNew->getTypeId()=='simple'){
                       // $newAssoProductsArray[]=$productIdWithoutLength;
                        ...
                        Mage::log('Found Item::'.$productIdWithoutLength, Zend_Log::DEBUG, 'parent_child_association.log', true);
                    }
                    else{
                        Mage::log('Item not Found::'.$withoutPostlengthSku, Zend_Log::DEBUG, 'parent_child_association.log', true);
                    }
                   // unset($productIdWithoutLength);
                }
            }else{
                Mage::log('No Lenth item found :: '.$child->getSku(), Zend_Log::DEBUG, 'parent_child_association.log', true);
            }
