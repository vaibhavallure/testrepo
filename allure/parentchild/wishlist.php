...
foreach ($wishListItemCollection as $item){
    ...
    if(preg_match("/MM/", $childSku) || preg_match("/mm/", $childSku)) {   //Check contains length
        ...
        if($withoutPostlengthProduct->getId() && in_array($configSku, $fixedItems)){
            ...
            $wishListOptionCollection = Mage::getModel('wishlist/item_option')->getCollection();
            $wishListOptionCollection->addFieldToFilter('wishlist_item_id', array('eq' =>$item->wishlistItemId()));
            
            foreach ($wishListOptionCollection as $wishListOption){
                
                if($wishListOption['code']=='info_buyRequest'){
                    print_r(unserialize($wishListOption['code']));
                    $buyRequest=unserialize($wishListOption['code']);
