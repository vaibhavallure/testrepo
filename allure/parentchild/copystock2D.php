...
foreach ($fixedItems as $fixedSku) {
    $count++;
    Mage::log('Count::'.$count.'-Parent SKU :: '.$fixedSku, Zend_Log::DEBUG, 'copystock_'.$name.'_2D.log', true);
    Mage::log('Stock ID  :: '.$stockId, Zend_Log::DEBUG, 'copystock_'.$name.'_2D.log', true);
            ...
            try {
                if(isset($newItemId)){
                    ...
                    if($newItemInventory->getId() && $oldItemInventory->getId()){
                        ...
                        Mage::log('Old product(Source) ID:: '.$oldItemId."--SKU::".$oldItem."---Qty=".$oldItemInventory->getQty(),  Zend_Log::DEBUG, 'copystock_'.$name.'_2D.log', true);
                        Mage::log('New Product(destination) ID:: '.$newItemId."--SKU::".$newItem."---Qty=".$newItemQty."---Total Qty::".$newItemInventory->getQty(), Zend_Log::DEBUG, 'copystock_'.$name.'_2D.log', true);
                        ...
                        Mage::log('Old product(Source) ID:: '.$oldItemId."-SKU::".$oldItem, Zend_Log::DEBUG, 'copystock_'.$name.'_2D_error.log', true);
                        Mage::log('New Product(destination) ID:: '.$newItemId."-SKU::".$newItem, Zend_Log::DEBUG, 'copystock_'.$name.'_2D_error.log', true);
                    }
                }
            } catch (Exception $e) {
                var_dump($e->getMessage());
                Mage::log('Exception Occured:: '.$e->getMessage(), Zend_Log::DEBUG, 'copystock_'.$name.'_2D_error.log', true);
            }
    ...
    Mage::log('====================================== ', Zend_Log::DEBUG, 'copystock_'.$name.'_2D.log', true);
   }
