...
foreach ($fixedItems as $fixedSku) {
	...
	foreach ($productCollection  as $product) {
		...
		Mage::log('Found Simple SKU :: '.$oldItem, Zend_Log::DEBUG, 'image_migrations.log', true);
		...
		if (count($oldItemSku) == 3) {
			...
			Mage::log('Original SKU :: '.$oldItem, Zend_Log::DEBUG, 'image_migrations.log', true);
			...
			if ($newItemId && !isset($imageUpdates[$newItemId])) {
				...
				foreach ($images as $image) {
				    ...
				    if( $path = $image->getPath() ) {
				        $newProduct->addImageToMediaGallery($path, array('image','thumbnail','small_image'), false, false);
				        $parentProduct->addImageToMediaGallery($path, array('image','thumbnail','small_image'), false, false);
				    }
				}
			
				Mage::log('New ITEM EXISTS !!', Zend_Log::DEBUG, 'image_migrations.log', true);
