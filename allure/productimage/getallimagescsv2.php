...
try {
    ...
    foreach ($results as $res) {
        ...
        $pro_sku=$res['sku'];
        $skuarray=explode("|",$res['sku']);

        if($skuarray[1])
        $pro_sku=$skuarray[0]."|".$skuarray[1];

        

        if (strpos($res['label'], "model") !== false || strpos($res['path'], "model") !== false) {
            /*$res['img_name']*/
            $pathArray_copy[2] = $pathArray[2] = str_replace(" ", "_", str_replace("|", "-", $pro_sku)) . "_model." . $ext;

            $keys = array_keys(array_column($datasku, 'img_name'), $pathArray[2]);

            $datasku[]['img_name']=$pathArray[2];

            if(count($keys)>0)
                $pathArray_copy[2] = $pathArray[2] = str_replace(" ", "_", str_replace("|", "-", $pro_sku)) . "_model_".count($keys)."." . $ext;
            ...
            /*$res['img_name']*/
            $pathArray_copy[2] = $pathArray[2] = str_replace(" ", "_", str_replace("|", "-", $pro_sku)) . "_" . $res['position'] . "." . $ext;
            ...
            if(count($keys)>0)
            $pathArray_copy[2] = $pathArray[2] = str_replace(" ", "_", str_replace("|", "-", $pro_sku)) . "_" . $res['position'] . "_".count($keys)."." . $ext;
