...
//        //this.accordion.openNextSection(true);
    ...
    setStepResponse: function(response){
        var shipping_fields = ['shipping:firstname','shipping:lastname','shipping:company','shipping:email','shipping:country_id','shipping:region_id','shipping:street1','shipping:street2','shipping:city','shipping:postcode','shipping:telephone','shipping:fax'];
        var billing_fields = ['billing:firstname','billing:lastname','billing:company','billing:email','billing:country_id','billing:region_id','billing:street1','billing:street2','billing:city','billing:postcode','billing:telephone','billing:fax'];
        var different = false;
        for(i=0; i<shipping_fields.length; i++){
            if($(shipping_fields[i]) && $(billing_fields[i])){
                if($(shipping_fields[i]).value != $(billing_fields[i]).value) {
                    different = true;
                    break;
                }
            }
        ...
        if (response.update_section) {
            ...
            if (jQuery.browser.msie && jQuery.browser.version === "8.0") {
                var bgImage =  this.skinUrl + 'frontend/mt/default/images/radio-button-checked.png';
                jQuery('#checkout-'+response.update_section.name+'-load input').each(function () {
                    if (jQuery(this).attr('checked'))
                        jQuery(this).next().css('background', 'url(' + bgImage + ') no-repeat left');
                    jQuery(this).bind('change', function() {
                        jQuery('#checkout-'+response.update_section.name+'-load dd label').attr('style', '');
                        if (jQuery(this).attr('checked')) {
                            jQuery(this).next().css('background', 'url(' + bgImage + ') no-repeat left');
                        }
                    });
...
Billing.prototype = {
    ...
    newAddress: function(isNew){ 
        Mage.Cookies.set('billing_add',$('billing-address-select').value);
        if (isNew) {
...
Shipping.prototype = {
    ...
    newAddress: function(isNew){
        ...
        if (isNew) {
            ...
            if ($('shipping:country_id') && $('shipping:postcode')) {
                if ($('shipping:country_id').value != 'US' && $('shipping:country_id').value != '') {
...
//        $('billing:use_for_shipping_yes').checked = flag;
        ...
        if ($('shipping:country_id') && $('shipping:postcode')) {
            if ($('shipping:country_id').value != '' && $('shipping:country_id').value != 'US') {
                $('shipping:postcode').removeClassName('validate-zip-international');
            } else {
                $('shipping:postcode').addClassName('validate-zip-international');
            }
        }
        if ($('shipping:postcode').next())
            $('shipping:postcode').next().remove(); // remove previous shipping post code validation advice if existed
    },
    ...
    syncWithBilling: function () {
        ...
        if (!$('billing-address-select') || !$('billing-address-select').value) {
            ...
            $('shipping:email_shipping').value = $('billing:email').value;
    ...
    nextStep: function(transport){
        ...
        if (response.update_section) {
            ...
            if (jQuery.browser.msie && jQuery.browser.version === "8.0") {
                ...
                jQuery('#checkout-'+response.update_section.name+'-load dt input').each(function () {
                    if (jQuery(this).attr('checked'))
...
Delivery.prototype = {
    ...
    save: function(){
        if (checkout.loadWaiting!=false) return;
        if (this.validate()) {
            checkout.setLoadWaiting('delivery-option');
            var request = new Ajax.Request(
                this.saveUrl,
                {
                    method:'post',
    ...
    nextStep: function(transport){
        ...
        if (response.update_section) {
            $('checkout-'+response.update_section.name+'-load').update(response.update_section.html);
            //display checked payment method
            jQuery('#checkout-payment-method-load dt').each(function(){
    			if(jQuery(this).find('input').is(':checked')){
    				var payMethod = jQuery(this).find('input').attr('value');
    				jQuery('#payment_form_'+payMethod).show();
    			}
    		});
        }
...
Payment.prototype = {
    ...
    save: function(){       
        if (checkout.loadWaiting!=false) return;
        var validator = new Validation(this.form);
        if(!jQuery('select[name="payment[cc_type]"]').val()){
            jQuery('#required-entry-cc-type').show();
        }else{
            jQuery('#required-entry-cc-type').hide();
        }
