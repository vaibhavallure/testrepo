	...
	var months = ['January','February','March','April','May','June','July','August','September','October','November','December'],
		...
		DCalendar = function(elem, options) {
			...
			if(options.mode === 'calendar')
				...
				this.tHead = $('<thead><tr><th colspan="5" id="currM"></th><th id="prev">&lsaquo;</th><th id="next">&rsaquo;</th></tr><tr><th>Su</th><th>Mo</th><th>Tu</th><th>We</th><th>Th</th><th>Fr</th><th>Sa</th></tr></thead>');
			...
			this.tHead = $('<thead><tr><th colspan="5" id="currM"></th><th id="prev">&lsaquo;</th><th id="next">&rsaquo;</th></tr><tr><th>S</th><th>M</th><th>T</th><th>W</th><th>T</th><th>F</th><th>S</th></tr></thead>');
			...
			this.calendar.on('click', '#next', function() { initCreate('next'); })
				...
				.on('click', '#today', function() {
					...
					if(that.options.mode === 'datepicker') {
						...
						//modified by bhagya selectedDate
						//$(this).addClass('selected');
						$('.date').removeClass('selectedDate');
		...
		create : function(mode){
			...
			if(mode === "days") {
				if(that.options.mode === 'calendar') {
					...
					that.tHead = $('<thead><tr><th colspan="5" id="currM"></th><th id="prev">&lsaquo;</th><th id="next">&rsaquo;</th></tr><tr><th>Su</th><th>Mo</th><th>Tu</th><th>We</th><th>Th</th><th>Fr</th><th>Sa</th></tr></thead>');
					...
					that.tHead = $('<thead><tr><th colspan="5" id="currM"></th><th id="prev">&lsaquo;</th><th id="next">&rsaquo;</th></tr><tr><th>S</th><th>M</th><th>T</th><th>W</th><th>T</th><th>F</th><th>S</th></tr></thead>');
				}
				...
				for(var i = 1; i <= 6; i++){
					...
					while(day <= days){
						...
						//PICKURDAY calendar disabled logic by BHAGYA
						
								//Get all the dates in mm/dd/yy formatted date  to match the json date and this formatted date Start 
								var date1 = new Date(d);
								var date1month = date1.getMonth()+1;
								var date1day = date1.getDate();
								
								if(Number(date1month) < 10 ){
									date1month = "0" + date1month;							
								}
								if(Number(date1day) < 10 ){
									date1day = "0" + date1day;							
								}
								...
								if(jsonDate != null)
								{
									...
									if ((that.minDate && d < min)){
										...
										temp[dayOfWeek].addClass('custom-disable');									
									}
									//Enable the dates which matches to working days json dates
								
									$.each(jsonDate, function (key, value){ 
 										if (alld == value) {
 											temp[dayOfWeek].removeClass('disabled');
 											temp[dayOfWeek].removeClass('custom-disable');
 											temp[dayOfWeek].removeClass('selectedDate');
 										}
									});
								}
								//To show disabled date for allocated Piercer end
								
						//PICKURDAY calendar disabled logic by BHAGYA
						
						//if ((that.minDate && d < min) || (that.maxDate && d > max) ) temp[dayOfWeek].addClass('disabled');
						//if(that.options.mode === 'datepicker' && d.getTime() == selected.getTime()) temp[dayOfWeek].addClass('selectedDate');
						...
						if (i === 1) {
							...
							for (var a = 6; a >= 0; a--) {
								if (temp[a].text() === ''){
									...
									if (that.options.mode === 'datepicker' && p.getTime() == selected.getTime()) temp[a].addClass('selectedDate');
								}
							}
						} 
						// Last week
							...
							for (var a = 0; a <= 6; a++) {
								if (temp[a].text() === ''){
									...
									if (that.options.mode === 'datepicker' && nextMonth.getTime() == selected.getTime()) temp[a].addClass('selectedDate');
								}
							}
						}
					}*/
				...
				this.tHead = $('<thead><tr><th colspan="2" id="currM"></th><th id="prev">&lsaquo;</th><th id="next">&rsaquo;</th></tr>');
